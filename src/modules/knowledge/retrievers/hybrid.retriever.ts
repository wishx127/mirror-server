...
export class HybridRetriever extends BaseRetriever {
  ...
  async _getRelevantDocuments(query: string): Promise<Document[]> {
    ...
    try {
      ...
      if (elapsed > 500) {
        this.logger.warn(`Hybrid search took ${elapsed}ms, consider optimizing`);
      }
  ...
  private async vectorSearch(query: string): Promise<VectorSearchResult[]> {
    try {
      ...
      try {
        const result = await client.query<{ id: number; content: string; similarity: number }>(
          `SELECT id, content, 
           ...
           LIMIT $4`,
          [queryEmbeddingString, this.userId, this.minSimilarity, this.limit * 2]
        );
  ...
  private async keywordSearch(query: string): Promise<KeywordSearchResult[]> {
    try {
      ...
      const keywordConditions = keywords
        .map((kw) => `CASE WHEN content ILIKE '%${this.escapeSQL(kw)}%' THEN 1 ELSE 0 END`)
        .join(" + ");
  ...
  private extractKeywords(query: string): string[] {
    ...
    const stopWords = new Set([
      ...
      "你", "会", "着", "没有", "看", "好", "自己", "这", "那",
      "什么", "怎么", "如何", "为什么", "哪些", "哪个", "请", "能",
      "可以", "帮", "帮我", "告诉", "介绍", "关于", "以及", "或者",
      "并且", "the", "a", "an", "is", "are", "was", "were", "be",
      "been", "being", "have", "has", "had", "do", "does", "did",
      "will", "would", "could", "should", "may", "might", "can",
      "must", "shall", "of", "to", "in", "for", "on", "with", "at",
      ...
      "into", "through", "what", "which", "who", "whom", "this", "that",
      "these", "those", "am", "it", "its", "my", "your", "his", "her",
      "their", "our", "i", "me", "you", "he", "she", "they", "we",
      "us", "him", "them",
    ]);
  ...
  async getSearchResults(query: string): Promise<SearchResult[]> {
    ...
    const elapsed = Date.now() - startTime;
    this.logger.log(`Search completed in ${elapsed}ms, returned ${mergedResults.length} results`);
